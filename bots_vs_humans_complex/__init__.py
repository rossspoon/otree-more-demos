...
Rather than using an ExtraModel, you could alternatively
store the bot decisions as Player fields,
but as (number of fields * number of bots per human) increases,
it becomes simpler to use an ExtraModel.
"""
...
class Player(BasePlayer):
    contrib = models.CurrencyField(min=0, max=C.ENDOWMENT, label="How much will you contribute?")
...
def set_payoffs(player: Player):
    ...
    player.group_individual_share = player.group_total_contrib * C.MULTIPLIER / C.AGENTS_PER_GROUP
